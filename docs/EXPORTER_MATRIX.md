...
|---|---|---|---|---|---|---|---|
| **Output** | Per-chat CSV | Per-chat CSV; EML via `--format eml` | Per-chat CSV | Per-chat CSV | Per-chat CSV (`__whatsapp` for WA) | Per-chat CSV (`__whatsapp`) | Per-chat CSV (also txt/html); EML/MBOX via `imessage-mail-exporter` |
...
**Mail archive format:** per-conversation folders of `.eml` with `X-ME-*` headers — see [`MAIL_ARCHIVE.md`](MAIL_ARCHIVE.md). SMS Backup & Restore (`--format eml`) and iMessage (`imessage-mail-exporter`: EML or MBOX) support MVP mail via `message-mail`; CSV remains the default.
