...
|---|---|---|---|---|---|---|---|
| **Output** | Per-chat CSV / EML / MBOX / **JSON IR** | Per-chat CSV / EML / MBOX / **JSON IR** | Per-chat CSV / EML / MBOX / **JSON IR** | Per-chat CSV / EML / MBOX / **JSON IR** | Per-chat CSV / EML / MBOX / **JSON IR** (`__whatsapp` for WA) | Per-chat CSV / EML / MBOX / **JSON IR** (`__whatsapp`) | Per-chat CSV (also txt/html); EML/MBOX via `imessage-mail-exporter` |
...
| **Source extras** | `pdu_*` (in `xml_fields_json`) | `subject`, `message_kind`, `xml_fields_json` | `smssync_id`, `eml_path` (in `xml_fields_json`) | `source_kind`, `has_attachments` (in `xml_fields_json`) | vendor date / status cols (in `xml_fields_json`) | `whatsapp_jid`, `whatsapp_key_id` (in `xml_fields_json`) | `parts_json`, `app_json`, … |
...
**Canonical IR:** [`MESSAGE_IR.md`](MESSAGE_IR.md) (`message-ir`). GO SMS Pro, SMS Backup & Restore, SMS Backup+, OpenExtract, iMazing, and WhatsApp all parse to `ConversationDocument` then project CSV/EML/MBOX/JSON via `message_ir::write_format`. Mail packaging: [`MAIL_ARCHIVE.md`](MAIL_ARCHIVE.md). CSV remains the default. Convert/compress/obfuscate stay CSV-only.
