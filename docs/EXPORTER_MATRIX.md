...
- Outgoing rows leave `sender_handle` blank by design (iMessage still fills `sender_display_name` with `"Me"` or caller-id)
...
| **iMessage** (`imessage-ir-exporter`) | Outgoing `sender_handle` blank; no WhatsApp; GPL-3.0-or-later; needs Mac/`chat.db` or iOS backup; no TXT/HTML |
