...
3. **macOS** (`macos-latest` only, when `MACOS_CERTIFICATE_BASE64` is non-empty): import the `.p12` into a temporary keychain, `codesign --options runtime --timestamp` each project binary, submit a zip of `message-exporter` to `notarytool --wait`, tear down the keychain, then package.
...
- macOS: `codesign -dv --verbose=4 ./message-exporter` and `spctl -a -vv ./message-exporter` (online Gatekeeper check).
