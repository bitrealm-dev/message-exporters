...
| **iMessage** (`imessage-ir-exporter`) | No WhatsApp; GPL-3.0-or-later; needs Mac/`chat.db` or iOS backup; no TXT/HTML |
...
**Re-export:** the [`message-reexporter` command](../../crates/message-ir/docs/REEXPORT.md), owned by `message-ir`, converts an existing Message Exporters output directory to another format (auto-detect input; GUI **Re-export** tab). Not a vendor backup source.
