...
**Convert:** the [`message-reexporter` command](../../crates/message-ir/docs/REEXPORT.md), owned by `message-ir`, converts an existing Message Exporters output directory to another format (auto-detect input; GUI **Convert** tab). Not a vendor backup source.
