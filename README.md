...
|-----------------|-----------|-------------------|-------------|
| **GO SMS Pro** local backup folder (Android) | [`go-sms-pro-exporter`](crates/go-sms-pro-exporter) | GO SMS Pro *(version TBD)* | [How messages become spreadsheet rows](crates/go-sms-pro-exporter/docs/XML_CSV_MAPPING.md) |
| **SMS Backup & Restore** XML from SyncTech (Android) | [`sms-backup-restore-exporter`](crates/sms-backup-restore-exporter) | SMS Backup & Restore **10.26.003** | [What the XML contains](crates/sms-backup-restore-exporter/docs/FIELDS.md), [How messages become spreadsheet rows](crates/sms-backup-restore-exporter/docs/XML_CSV_MAPPING.md) |
...
| **OpenExtract** conversation CSV + contacts `.vcf` | [`openextract-exporter`](crates/openextract-exporter) | OpenExtract **0.5.1** | [Converter README](crates/openextract-exporter/README.md), [example spreadsheet](crates/openextract-exporter/sample-output/_15555550122.csv) |
| **iMazing** Messages / WhatsApp CSV + Contacts CSV | [`imazing-exporter`](crates/imazing-exporter) | iMazing **3.5.5** | [Converter README](crates/imazing-exporter/README.md), [Design & limitations](crates/imazing-exporter/docs/DESIGN.md), [example spreadsheet](crates/imazing-exporter/sample-output/_13212462167.csv) |
...
Raw **iMazing** vendor Messages CSV can also be ingested by [message-vault-rs](https://github.com/bitrealm-dev/message-vault-rs) `csv-ingest` (no contact enrichment). Prefer [`imazing-exporter`](crates/imazing-exporter) when you have the Contacts export (Messages and/or WhatsApp). To share structure without PII, rewrite vendor CSV with [`imazing-obfuscate`](crates/message-obfuscate).
...
Or put phone/email in [`crates/sms-backup-plus-exporter/config/owner.toml`](crates/sms-backup-plus-exporter/config/owner.example.toml) instead of the flags. `--contacts` or `--vcf` is recommended so names resolve; without it a warning is printed.
