...
```bash
imessage-exporter -f csv -c clone -o csv_export
```
...
2. Restore the CSV overlay: `src/exporters/csv/`, `ExportType::Csv`, binary/package rename to `imessage-exporter`, crates.io `imessage-database`, default `-f csv`
3. Smoke: `cargo build -p imessage-exporter && imessage-exporter -f csv -o /tmp/out`
