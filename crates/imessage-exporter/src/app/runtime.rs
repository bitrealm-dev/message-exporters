        ...
        if self.options.diagnostic {
            ...
            match export_type {
                ...
                ExportType::Csv => {
                    ...
                    if self.options.obfuscate || self.options.obfuscate_seed.is_some() {
                        let mut anon = message_obfuscate::resolve_obfuscator(
            ...
            if !matches!(export_type, ExportType::Csv)
                && (self.options.obfuscate || self.options.obfuscate_seed.is_some())
            {
                eprintln!("Warning: --obfuscate only applies to CSV exports; skipped.");
            }
        }
        println!("Done!");
        Ok(())
