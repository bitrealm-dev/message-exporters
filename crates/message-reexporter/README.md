...
Auto-detects a single input format among `csv`, `eml`, `mbox`, `json`, `jsonl`, and `xml` (`smses.xml`), loads conversations into canonical IR, then writes via `FormatSink` (media modes + obfuscate included).
