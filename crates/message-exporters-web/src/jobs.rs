//! In-process exporter job adapters for the web GUI.
//!
//! This mirrors `message-exporters-gui/src/jobs.rs` exactly. It is duplicated
//! (rather than shared via `message-exporters-core`) so the native GUI crate
//! is not touched by the web port.
...
use message_exporters_core::{CancelFlag, Exporter, ExporterConfig, LogSink, ProcessEvent};
...
/// Install cancel + live log sink, then run an exporter library entrypoint.
fn prepare_config(mut config: ExporterConfig, cancel: CancelFlag, tx: &mpsc::Sender<ProcessEvent>) -> ExporterConfig {
...
/// Attach cancel + live log sink for re-export (and similar) jobs.
