...
use sms_backup_plus_export::convert_export;

#[derive(Parser, Debug)]
#[command(name = "sms-backup-plus-export")]
    ...
    match cli.command {
        Commands::Convert {
            ...
            if media_mode.needs_tools() {
                ...
                let media = process_export_media(&output, media_mode, &compress)?;
                eprint_report(&media);
            ...
            if obfuscate || obfuscate_seed.is_some() {
                ...
                let n = obfuscate_export_dir(&output, &mut anon)?;
