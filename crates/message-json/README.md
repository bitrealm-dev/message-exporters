...
CSV → vault NDJSON conversion lives in [`message-vault-rs`](https://github.com/bitrealm-dev/message-vault-rs) (`csv-ingest`). This crate only defines the wire types exporters write (SMS / iMessage NDJSON) and the vault schema types for documentation alignment.

## Out of scope

Phone normalization, emoji decoding, SMIL/MMS parsing, and archive-format readers stay in the exporter crates in this repo.
